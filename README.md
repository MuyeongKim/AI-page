...
- `V26.0908` (2026-09-08)
- GUI 창 제목·프로그램 안내와 공개 릴리스 피드는 같은 버전을 사용합니다.
- [Windows 배포본 ZIP 다운로드](https://drive.google.com/file/d/1nrEi7MmLmVYaYK33hloeHfGYxx5tmjcV/view?usp=sharing): `AI객체탐지프로그램 V26.0908(디렉토리).zip` (약 2.14 GB). 파일 크기와 SHA-256은 [릴리스 피드](latest_version.json)에 기록합니다.
- ZIP 파일에서 **모두 압축 풀기**를 선택한 뒤 같은 이름의 폴더 안에서 `AI객체탐지프로그램 V26.0908(디렉토리).exe`를 실행하세요. 실행 파일·`_internal` 구성 폴더·모델 파일을 같은 위치에 보관해야 합니다.
...
- V26.0908 Windows 배포 ZIP을 Google Drive 다운로드 버튼에 연결하고 파일명·실제 크기·SHA-256을 공개했습니다. 공개 링크에서 파일 전체를 내려받아 ZIP CRC 검사를 통과했으며, 포함된 PDF가 웹 설명서 원본과 일치하는지 확인했습니다. 전체 압축 해제와 실행 파일·구성 폴더·모델 파일 보관 방법도 안내합니다.
- 소개 웹사이트에 최신 Windows 실행 화면과 PDF 사용설명서 보기·다운로드를 반영했습니다. 실행별 결과 폴더와 **작업 결과 → 저장 폴더 열기** 안내도 갱신했습니다. 웹은 설명서의 이미지·PDF 원본을 함께 사용하며, 버전 불일치나 누락된 다운로드는 빌드 검증에서 확인합니다.

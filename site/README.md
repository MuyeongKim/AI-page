...
현재 실행 화면은 `docs/images/manual-main-V26.0908.png`의 Windows 화면입니다.
사용설명서와 같은 원본을 Astro에서 직접 가져오며, 이전 시연 사진과 구분해
입력·실행·결과 확인 순서를 설명합니다. GUI를 바꾸면 실제 실행 화면도 함께 갱신하고
개인 파일 경로나 인증 정보가 포함되지 않았는지 확인합니다.

## 사용설명서와 버전 갱신

`output/pdf/Stay-Up-AI-사용설명서-V26.0908.pdf`를 ‘사용설명서 보기’와 ‘PDF 다운로드’로
제공합니다. 사진·영상 실행마다 시작 시각별 새 결과 폴더를 만들며, **작업 결과 →
저장 폴더 열기**로 확인하는 흐름을 화면 설명과 FAQ에 반영했습니다. Windows 배포본의
공개 여부는 `latest_version.json`의 다운로드 상태를 계속 따릅니다.

새 버전에서는 `src/pages/index.astro`의 `guideVersion`, Windows 화면과 PDF import를
함께 갱신합니다. `guideVersion`이 릴리스 피드와 다르면 빌드를 중단합니다. 빌드 검증은
화면·PDF가 해당 버전의 원본과 같은지, 다운로드 파일명과 링크가 올바른지 확인합니다.
...
수정·게시·게시 중단·삭제 요청은 화면에서 확인한 리비전을 함께 보냅니다. 다른 화면에서
변경한 내용이 먼저 저장되면 서버가 `409`로 거부합니다. 편집 중 충돌하면 입력 내용은
유지되고 최신 저장본을 비교할 수 있습니다. 필요한 내용을 입력란에 반영한 뒤
‘비교한 내용으로 수정 계속’과 ‘변경 저장’을 누릅니다. 목록의 게시·삭제에서 충돌하면
‘새로고침’으로 최신 내용을 확인한 뒤 다시 시도합니다.

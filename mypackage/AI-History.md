...
**성능 최적화 및 UX 개선**  
- **스레드 분리(QThread 적용)**: YOLO 탐지 로직을 백그라운드 스레드로 분리하여 UI 멈춤 현상(Freezing) 완전 해결.  
- **영상 창 크기 조절 기능**: `cv2.WINDOW_NORMAL` 적용으로 실시간 탐지 창의 크기 조절 가능 (기본 1280x720).  
- **코드 리팩토링**: `DetectionWorker` 클래스 도입으로 유지보수성 향상.
